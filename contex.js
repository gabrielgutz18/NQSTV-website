...
const supportChatWidget = document.getElementById('support-chat-widget');
...
const supportChatText = document.getElementById('support-chat-text');
...
if (
    ...
    const supportReplies = [
        'Thanks for reaching out. Our team can help with your project or training needs.',
        'Sure, we would be happy to assist. Please share a few details so we can guide you properly.',
        'We received your message. You can also use the contact form below for a full inquiry.',
        'No problem. Tell us what service you are interested in and we will point you in the right direction.',
        'Happy to help. If you want, you can also leave your contact details in the form below.'
    ];

    const setSendState = () => {
        supportChatSend.disabled = supportChatText.value.trim().length === 0;
    };
    ...
    const appendMessage = (text, type) => {
        ...
        message.className = `support-chat-message support-chat-message--${type}`;
    ...
    supportChatToggle.addEventListener('click', openSupportChat);
    ...
    supportChatText.addEventListener('input', setSendState);

    supportChatText.addEventListener('keydown', (event) => {
        if (event.key === 'Enter') {
            event.preventDefault();
            sendSupportMessage();
        }
    ...
    document.addEventListener('click', (event) => {
        if (
            supportChatWidget.classList.contains('is-open') &&
            event.target instanceof Node &&
            !supportChatWidget.contains(event.target)
        ) {
            closeSupportChat();
        }
    });
    ...
    document.addEventListener('keydown', (event) => {
        if (event.key === 'Escape' && supportChatWidget.classList.contains('is-open')) {
            closeSupportChat();
        }
    });
